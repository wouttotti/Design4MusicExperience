...
public class JointOrientation : MonoBehaviour
{
    ...
    void Update ()
    {
        ...
        if (updateReference && !Calibrated) {
            ...
            StartPosition = new Vector2(myo.transform.forward.x, myo.transform.forward.y);
        ...
        Quaternion rotation = transform.rotation = _antiYaw * antiRoll * Quaternion.LookRotation (myo.transform.forward);

        Vector3 position = rotation.ToEulerAngles();
        Pointer.transform.position = new Vector3(position.y * 3, -position.x * 3, 0);
        ...
        if (thalmicMyo.xDirection == Thalmic.Myo.XDirection.TowardWrist) {
            ...
            Quaternion rotationWrist = transform.rotation = new Quaternion(transform.localRotation.x,
                                                -transform.localRotation.y,
                                                transform.localRotation.z,
                                                -transform.localRotation.w);
            Vector3 positionWrist = rotationWrist.ToEulerAngles();
            Pointer.transform.position = new Vector3(position.y * 3, -position.x * 3, 0);
        }
    }
